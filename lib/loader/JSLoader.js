 ...
 * @constructor
 * Extracts options from the docblock, calculates gziped network size.
 * Both javalin symbols parsing and network size calculation are off
 * by default due to their perf cost.
 * Use options parameter to switch them on.
 *
...
JSLoader.__proto__ = ResourceLoader;
...
JSLoader.prototype.postProcess = function(map, resources, callback) {
  ...
  // spend calling `findAbsolutePathForRequired`.
  // 这个缓存只对非相对路径有效. If relative requires ('.') the calling file
   ...
   * @param {string} type
   */
  function fixModuleID(r, type) {
    var required = type === 'JS' ?
        r.requiredModules :
        (type === 'CSS' ? r.requiredCSS : []);
    var resource, i;
    for (i = 0; i < required.length; i++) {
      var requiredText = required[i];
      resource = map.getResource('JS', requiredText) || map.getResource('CSS', requiredText);
