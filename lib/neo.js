 ...
 *   {?Number} options.maxOpenFiles loaders的最大数目，MapUpdateTask用到
 *   {?Number} options.maxProcesses   Maximum number of loader forks MapUpdateTask can use
 *   {?Boolean} options.useNativeFind  使用linux系统的shell命令(faster)还是node实现的方法(safer)
 *   {?Function:Boolean} options.ignorePaths  返回是否忽略路径的函数
