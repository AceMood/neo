/**
 * @file 对css内容解析时对属性值的匹配
 * @author AceMood
 * @email zmike86@gmail.com
 */

'use strict';

// Below is value
// background: url("../img/a.png");
// background: url("../img/a.png"), url("../img/a.png");
// behavior: url(ul.htc) url(hilite.htc);
// @font-face {
//   font-family: "my-style";
//   src: url('a.eof'),
//     url('a.ttf'),
//     url('a.woff');
// }
var RE_URL = /(?:\s*)url\((?:(?:"|')?)(?:\s*)([^\)"']*)(?:\s*)(?:(?:"|')?)\)(?:\s*)/;

// filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled='true', sizingMethod='corp', src='top.png');
var RE_SRC = /src=(?:(?:"|')?)(?:\s*)([^\)"']*)(?:\s*)(?:(?:"|')?)/;

//'@import url(a.css)';
//'@import "a.css";
var RE_IMPORT_URL = /(?:\s*)(?:url\("?|")(?:\s*)([^\)"]*)(?:\s*)(?:"|"?\))(?:\s*)/;


// Below is css property
// background: url("../img/a.png");
var BACKGROUND_IMAGE = /^(_|\*)?(-(o|webkit|ms|moz)-)?background(-image)?$/;

// background: url("../img/a.png"), url("../img/a.png");
var MULTI_BACKGROUND_IMAGE = /^background$/;

// border-image: url(../img/a.png) 30 30 round;
// border-image-source: url(../img/a.png);
var BORDER_IMAGE = /^(-(o|webkit|ms|moz)-)?border-image(?:\-source)?$/;

// behavior: url(ul.htc) url(hilite.htc)
// see: `http://www.w3chtml.com/css3/properties/only-ie/behavior.html`
var BEHAVIOR = /^(_|\*)?behavior$/;

// -ms-filter: 'progid:DXImageTransform.Microsoft.MotionBlur(strength=50), progid:DXImageTransform.Microsoft.BasicImage(mirror=1)';
// filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled='true', sizingMethod='corp', src='top.png');
var FILTER = /^(_|\*|\-ms\-)?filter$/;

//'@import url(backgroundImage.css)';
var IMPORT_PATH = /import/;


// 导出
exports.RE_URL = RE_URL;
exports.RE_IMPORT_URL = RE_IMPORT_URL;
exports.RE_SRC = RE_SRC;
exports.BACKGROUND_IMAGE = BACKGROUND_IMAGE;
exports.MULTI_BACKGROUND_IMAGE = MULTI_BACKGROUND_IMAGE;
exports.BORDER_IMAGE = BORDER_IMAGE;
exports.BEHAVIOR = BEHAVIOR;
exports.FILTER = FILTER;
exports.IMPORT_PATH = IMPORT_PATH;

exports.getBgImages = function(val) {
  var parts = val.split(',');
  var ret = [];
  parts.forEach(function(part) {
    var r = part.match(RE_URL);
    if (r && r[1]) {
      ret.push(r[1]);
    }
  });
  return ret;
};
exports.getFilters = function(val) {
  var ret = [];
  var r = val.match(RE_SRC);
  if (r && r[1]) {
    ret.push(r[1]);
  }
  return ret;
};
exports.getHTCs = function(val) {
  var parts = val.split(/\s+/);
  var ret = [];
  parts.forEach(function(part) {
    var r = part.match(RE_URL);
    if (r && r[1]) {
      ret.push(r[1]);
    }
  });
  return ret;
};
exports.getImportUrl = function(val) {
  var ret = [];
  var r = val.match(RE_IMPORT_URL);
  if (r && r[1]) {
    ret.push(r[1]);
  }
  return ret;
};
exports.getWebFonts = function(val) {
  var parts = val.split(/,/);
  var ret = [];
  parts.forEach(function(part) {
    var r = part.match(RE_URL);
    if (r && r[1]) {
      ret.push(r[1]);
    }
  });
  return ret;
};