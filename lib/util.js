 ...
 * @returns {string}
 */
exports.normalizeSysPath = function(path) {
  // 传来的参数有可能是path.normalize得来的，
  // 这个方法会把http开头的绝对路径中协议部分替换成单个`/`
  if (exports.isAbsUrl(path)) {
    return path.replace(/^(https?:\/)(?:\/)?(.*)/, function($0, $1, $2) {
      var np = node_path.normalize($2);
      if (np.charAt(0) === '/') {
        return $1 + $2;
      } else {
        return $1 + node_path.sep + $2;
      }
    })
  } else if (process.platform === 'win32') {
    return node_path.normalize(p);
  } else {
    return node_path.normalize(p).replace(/\\/g, '/');
  }
};